...
bool RenderFrameHostManager::IsRendererTransferNeededForNavigation(
  ...
  GetCoopCoepCrossOriginIsolationInfo(request,
                                      ...
                                      &coop_coep_cross_origin_isolated_origin);
  if (ShouldSwapBrowsingInstancesForDynamicIsolation(
          rfh, dest_url_info, is_coop_coep_cross_origin_isolated,
          coop_coep_cross_origin_isolated_origin))
    return true;
  ...
  if (rfh->GetSiteInstance()->RequiresDedicatedProcess() ||
      SiteInstanceImpl::DoesSiteInfoRequireDedicatedProcess(
          ...
          SiteInstanceImpl::ComputeSiteInfo(
              rfh->GetSiteInstance()->GetIsolationContext(), dest_url_info,
              is_coop_coep_cross_origin_isolated,
              coop_coep_cross_origin_isolated_origin))) {
    return true;
  }
...
bool RenderFrameHostManager::CanSubframeSwapProcess(
  ...
  if (!IsRendererTransferNeededForNavigation(
          render_frame_host_.get(), UrlInfo(resolved_url, true), request)) {
