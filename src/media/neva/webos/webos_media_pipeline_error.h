...
#ifndef MEDIA_BASE_NEVA_WEBOS_WEBOS_MEDIA_PIPELINE_ERROR_H_
#define MEDIA_BASE_NEVA_WEBOS_WEBOS_MEDIA_PIPELINE_ERROR_H_

enum WebosMediaPipelineStatus {
...
#define SMP_STATUS_IS_1000_PIPELINE_ERROR(status) \
  ((status) == SMP_PIPELINE_DIED_ABNORMALLY)

#define SMP_STATUS_IS_40000_STREAMING_ERROR(status) \
  ((status) >= SMP_STREAMING_PROTOCOL_RELATED_ERROR && (status) < 50000)

#endif  // MEDIA_BASE_NEVA_WEBOS_WEBOS_MEDIA_PIPELINE_ERROR_H_
