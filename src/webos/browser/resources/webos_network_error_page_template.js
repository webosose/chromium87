    ...
    var interval = setInterval(
      function() {
        ...
        palmObject.onservicecallback = function(msg) {
          ...
          isSupport = response.configs['tv.hw.atsc30legacybox'] ===
              "ATSC30_LEGACYBOX_SUPPORT";
...
function setButtonStyle() {
  console.log("Application Viewport: [" + appResolutionWidth +", " +
      appResolutionHeight +"]");
  ...
  for (i = 0; i < buttons.length; i++) {
    buttons[i].style.height =
        ((appResolutionWidth * buttons_height) / (screen.width)) + size_unit;
    buttons[i].style.lineHeight =
        ((appResolutionWidth * buttons_lineHeight) / (screen.width)) +
        size_unit;
    buttons[i].style.borderRadius =
        ((appResolutionWidth * buttons_borderRadius) / (screen.width)) +
        size_unit;
    buttons[i].style.fontSize =
        ((appResolutionWidth * buttons_fontSize) / (screen.width)) + size_unit;
  }
}
...
function setPageStyle() {
  ...
  errordetails.style.fontSize =
      ((appResolutionWidth * errordetails_fontSize) / (screen.width)) +
      size_unit;

  // Error info
  errorinfo.style.fontSize =
      ((appResolutionWidth * errorinfo_fontSize) / (screen.width)) + size_unit;

  // Error guide
  errorguide.style.fontSize =
      ((appResolutionWidth * errorinfo_fontSize) / (screen.width)) + size_unit;
}

function setErrorPageMessages(
    error_title, error_details, error_guide, error_info) {
