...
void EmulatorDataSource::FetchURLOnce(const std::string& fetch_url) {
  ...
  // FIXME(neva): net::URLFetcher was deprecated in upstream
  // https://chromium-review.googlesource.com/c/chromium/src/+/2163258
...
void EmulatorDataSource::PeriodicPoll() {
  ...
  task_runner()->PostDelayedTask(
      FROM_HERE, base::Bind(&EmulatorDataSource::PeriodicPoll, GetWeakPtr()),
...
void EmulatorDataSource::SetExpectationAsync(const std::string& url,
  ...
  instance->task_runner()->PostTask(
      FROM_HERE, base::Bind(&EmulatorDataSource::SetExpectation,
...
bool EmulatorDataSource::GetResponseParams(ResponseArgs &args,
  ...
  if (!ret.value) {
    LOG(ERROR) << __func__ << "() : JSONReader failed : " << ret.error_message;
    return false;
  }

  if (!ret.value->is_dict()) {
  ...
  const base::DictionaryValue& response_object =
        static_cast<base::DictionaryValue&>(*ret.value);

  for (auto arg: args) {
    if (!response_object.GetString(arg.name, arg.value)) {
      LOG(ERROR) << __func__ << "() : Absent argument '" << arg.name << "'";
      return false;
    }
  }
  return true;
...
void EmulatorDataSource::OnURLFetchComplete(const net::URLFetcher* source) {
  std::unique_ptr<const net::URLFetcher> fetcher(source);
  {
    ...
    if (expectation_result != expectation_fetchers_.end()) {
      const net::Error error = source->GetError();
      if (error) {
